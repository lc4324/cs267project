...
@register_dataset('fma_node_classification')
class FMA_NodeClassification(NodeClassificationDataset):
    r"""
    The RDF dataset will be used in task *entity classification*.
    Dataset Name : aifb/ mutag/ bgs/ am.
    We download from dgl and process it, refer to
    ...
    def __init__(self, dataset_name, *args, **kwargs):
        super(FMA_NodeClassification, self).__init__(*args, **kwargs)
        self.g, self.category, self.num_classes = self.load_FMA_dgl()
        self.has_feature = False

    def load_FMA_dgl(self):

        kg_dataset = FMADataset()
